...
\section{The Notion of a Form}
	...
	\index{Exponential Map|(}
		...
		\begin{defn}[Pushforward of a Vector Field]
			For a vector field $X \in TM$ we define its pushforward $\varphi_*(X) \in TN$ by its action on $f \in C^\infty(N)$
			\begin{equation*}
				\varphi_*(X)(f) = X (\varphi^*(f))
			\end{equation*}
		\end{defn}
		To understand this better let's do a small example. 
		
		\textbf{INSERT PUSHFORWARD EXAMPLE HERE}
		
		Being able to push forward vector fields along $\varphi$ will be very useful. Now we will specialize to when $\varphi = \varphi_X(t): M \rightarrow M$ is a flow associated with a vector field $X$.\\
...
Nothing stops us from going from our starting point $p$, moving along $X$ by $dt$ and at least seeing how the flow of $Y$ looks there, namely:
	...
	If we write out the flow of $\gamma'(s)$ explicitly through exponential maps and let $t\rightarrow 0$, we can get an easier definition
	\begin{align*}
		\varphi_X (\varphi_Y(\varphi_X(p, dt), s), -dt) = e^{-dt X} e^{s T} e^{dt X} \\
		&= (1 - dt X)(1 + s Y) (1 + dt X) =
	\end{align*}
