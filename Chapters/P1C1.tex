...
\section{The Notion of a Form}
	...
	Because there is only one differential multiplying each term (be it $dx$ or $dy$), we call such $\omega$ \textbf{1-forms}. \index{Differential Form!1-Form} It is easy to show \textbf{MAKE AN EXERCISE} that the sum of one-forms is still a one-form, and that multiplying a one-form by a function keeps it as a one-form. 
